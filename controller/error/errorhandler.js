...
const handleJWTExpiredError = () =>
  new AppError(ERRORS.UNAUTHORIZED.EXPIRED_JWT, 401);
...
const sendErrorPro = (err, req, res) => {
  ...
  return res.status(STATUS_CODE.SERVER_ERROR).json({
    status: STATUS.ERROR,
    message: ERRORS.PROGRAMMING.SOME_ERROR,
  });
};
module.exports = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    if (err.type === "expressValidationError")
      err = handleValidationErrorExpress(err);
